    ...
    private function sysCommandLine($v, $command) {
        ...
        if (isset($command[$v['type']][$v['command']][$v['protocol']])) {
            ...
            if ($v['type'] == 'Multi' || $v['type'] == 'MultiDouble')
                $sysStr = str_replace('?pass?', $this->getPass($v['order_id']), $sysStr);
            ...
            $this->logLaunchScript($v['command'], $sysStr, $retval, $v['order_id']);
            return $r;
        }
    }
...
//            }
    ...
    private function setAccount() {
        ...
        if ($this->_accountPass) {
            ...
            $this->q->query('Select a.id From accounts a 
						...
						' . (($this->hostname) ? 'and s.hostname = "' . $this->hostname . '"' : '') . '
                             Where a.name="' . $login . '"');
            if ($this->q->numrows() > 0) {
                $res=$this->q->fetch_data_to_array();
                foreach ($res as $d)
    ...
    private function logLaunchScript($action, $command_line, $retval, $order_id) {
        if (!$this->q->query('Insert Into log_lounch_script (action,comman_line,return_val,order_id)
            Values ("' . $action . '","' . $command_line . '","' . $retval . '",' . $order_id . ')')) {
            throw new Exception;
        }
    }
